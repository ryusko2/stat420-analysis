...
passengers ~ origin_state + dest_state + intra_subr + 
##     seats + flights + dist + year + month.chr + I(seats^2) + 
##     I(flights^2) + I(dist^2) + I(dest_pop^2) + seats:flights
...
* $x_2$ = `dest_state` = \{ AL, AL, AR, AZ, CA, CO, CT, DC, DE, FL, GA, HI, IA, ID,
                              ...
                              ND, NE, NH, NJ, NM, NV, NY, OH, OK, OR, PA, RI, SC, SD,
                              TN, TX, UT, VA, VT, WA, WI, WV, WY \}
* $x_3$ = `intra_sub` = $\begin{cases}
                            \text{TRUE,} & \text{if flight is within subregion}\\
                            \text{FALSE,} & \text{if flight arrived in different subregion}\\
                          \end{cases}$
* $x_4$ = `seats` (numeric)
* $x_5$ = `flights` (numeric)
* $x_6$ = `dist` (numeric)
...
###Prediction case study, NY-FL

```{r}
#predict(flight_model_quad_bwd_aic, newdata = data.frame(origin_state = "NY", dest_state = "FL", ))
```

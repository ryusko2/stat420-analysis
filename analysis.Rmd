...
Based on a recommendation overheard in help sessions, we decided to start modeling with a subset of 20,000 observations, without any restrictions or geographical limitation. We used both training and test datasets. Due to having categorical variables in the dataset with a large number of levels (i.e. `origin_state`), the training dataset must contain all the levels in order for the `predict()` function to work on all test datasets. The statistically sound approach to this would be to determine what proportion each level is represented in the full dataset and then do a stratified random sample of each of those levels. However, to simplify this problem, what we chose to do was take a large random sample and then check that all levels were in fact captured in the sample. If not, just sample again until we have a sample with all levels.

Also the simple `sample()` function does not guarantee the training and testing datasets are unique, but sampling out of 3.2 million observations shouldn't overlap much.
...
Although we originally planned on using regions to reduce the complexity of the full dataset, with sampling of the full dataset, we left `origin_state` and `dest_state` as yet another possible "region" division, and found that these large categorical predictors do fine tune the prediction better.
...
Looking at the numeric predictor values, we can see that `passengers`, `seats`, and `flights` are all highly correlated.
...
flight_model_mod = lm((passengers ^ lambda - 1) / lambda ~ 
...
#step selection
flight_model_mod_bwd_aic = step(flight_model_mod, direction = "backward", trace = 0)
flight_model_mod_bwd_bic = step(flight_model_mod, direction = "backward", k = log(n), trace = 0)
...
train_rmse_mod = calc_rmse(flight_sub_train$passengers -
                           reverse_transform(predict(flight_model_mod)))

test_rmse_mod  = calc_rmse(flight_sub_test$passengers -
                           reverse_transform(predict(flight_model_mod, flight_sub_test)))

train_rmse_mod_bwd_aic = calc_rmse(flight_sub_train$passengers -
                                   reverse_transform(predict(flight_model_mod_bwd_aic)))

test_rmse_mod_bwd_aic  = calc_rmse(flight_sub_test$passengers -
                               reverse_transform(predict(flight_model_mod_bwd_aic, flight_sub_test)))

train_rmse_mod_bwd_bic = calc_rmse(flight_sub_train$passengers -
                                   reverse_transform(predict(flight_model_mod_bwd_bic)))

test_rmse_mod_bwd_bic  = calc_rmse(flight_sub_test$passengers -
                                   reverse_transform(predict(flight_model_mod_bwd_bic, flight_sub_test)))

#List results
summary(flight_model_mod_bwd_aic)$call
summary(flight_model_mod_bwd_bic)$call

diagnostics(flight_model_mod_bwd_aic, testit = FALSE)
```
...
flight_model_big_bwd_bic | `r summary(flight_model_big_bwd_bic)$adj.r.squared` | `r train_rmse_big_bwd_bic` | `r test_rmse_big_bwd_bic`
flight_model_best_guess | `r summary(flight_model_best_guess)$adj.r.squared` | `r train_rmse_best_guess` | `r test_rmse_best_guess`
flight_model_mod | `r summary(flight_model_mod)$adj.r.squared` | `r train_rmse_mod` | `r test_rmse_mod`
flight_model_mod_bwd_aic | `r summary(flight_model_mod_bwd_aic)$adj.r.squared` | `r train_rmse_mod_bwd_aic` | `r test_rmse_mod_bwd_aic`
flight_model_mod_bwd_bic | `r summary(flight_model_mod_bwd_bic)$adj.r.squared` | `r train_rmse_mod_bwd_bic` | `r test_rmse_mod_bwd_bic`
...
```{r, warning=FALSE}
n = length(resid(flight_model_mod_bwd_aic))

percent = sum(cooks.distance(flight_model_mod_bwd_aic) >
              (4 / length(cooks.distance(flight_model_mod_bwd_aic))), na.rm = TRUE) / n * 100

plot(flight_model_mod_bwd_aic, which = 6)
...
Our best model so far as been the `flight_model_big_bwd_aic` which started with all predictors and then used a backwards search using an AIC criteria for selecting $\beta$ parameters. We will start back with that model and add an interaction between the highly correlated `seats` and `flights`.

```{r}
#All predictors plus and interaction between seats and flights
flight_model_int = lm(passengers ~ . + seats * flights, data = flight_sub_train)
...
- `Passengers`, `Seats` and `Flights` are so hightly correlated that using `Seats` or `Flights` as the response variable did not add anything to the data analysis.
- We were hoping to *observe* a better relationship between `passengers` and either `origin_pop` or `dest_pop`, but neither shows a distinguishable pattern in this sample.
